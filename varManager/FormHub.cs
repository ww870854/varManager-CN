...
namespace varManager
{
    public partial class FormHub : Form
    {
        ...
        {
            string message = "扫描HUB以查找所有已整理变量中缺失的依赖项。将生成一个下载链接列表。您必须先登录 hub.virtamate.com 才能下载这些链接，建议使用 Chrono for Chrome 进行下载。";
        ...
        private async void AllMissingDepends()
        {
            ...
            if (missingvars.Count > 0)
            {
                ...
                this.BeginInvoke(addlog, new Object[] { " 在HUB中搜索可下载的依赖项.", LogLevel.INFO });
                ...
                {
                    this.BeginInvoke(addlog, new Object[] { $"{packageArray.Count} 返回记录将被分析", LogLevel.INFO });
                    foreach (var package in packageArray.Childs)
                    {
                        ...
                        if (!string.IsNullOrEmpty(downloadurl) && downloadurl != "null")
                        {
                            if (!string.IsNullOrEmpty(filename) && filename != "null")
                            {
                                ...
                                {
                                    this.BeginInvoke(addlog, new Object[] { $"在HUB中查找{filename}", LogLevel.INFO });
                ...
                {
                    this.BeginInvoke(addlog, new Object[] { $"共找到 {downloadUrls.Count} 个下载链接", LogLevel.INFO });
                    ShowDownList();
                ...
                {
                    this.BeginInvoke(addlog, new Object[] { "未找到下载链接", LogLevel.INFO });
                }
            }
        }
        ...
        {
            string message = "扫描HUB用于获取更新包。系统将生成一份下载链接列表。您必须先登录 hub.virtamate.com 才能下载这些链接，建议使用 Chrono for Chrome 进行下载。";
        ...
        private async void UpdatAllPackages()
        {
            ...
            foreach (var hubpackage in hubPackages)
            {
                ...
                if (varlastname != "missing")
                {
                    ...
                    if (splitindex >= 0)
                    {
                        ...
                        {
                            this.BeginInvoke(addlog, new Object[] { $"查找可升级的软件包,{varlastname} ", LogLevel.INFO });
            ...
            if (toBeUpdVars.Count > 0)
            {
                ...
                this.BeginInvoke(addlog, new Object[] { $"在HUB中总共发现{toBeUpdVars.Count}个可升级的变量文件。", LogLevel.INFO });
                ...
                {
                    this.BeginInvoke(addlog, new Object[] { "未找到下载链接", LogLevel.WARNING });
                }
            }
        }
        ...
        private void buttonCopytoClip_Click(object sender, EventArgs e)
        {
            if (downloadUrls.Count > 0)
            {
                ...
                MessageBox.Show("已复制到剪贴板，可粘贴至Chrome（Edge）浏览器的Chrono下载器中进行下载");
            }
        }
        ...
        private void FormHub_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                if (MessageBox.Show("下载列表数据将丢失，确认退出？", "警告",
