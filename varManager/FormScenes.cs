...
namespace varManager
{
    public partial class FormScenes : Form
    {
        ...
        */
            ...
            if (listView.SelectedIndices.Count == 1)
            {
                ...
                if (item != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(item.SubItems[3].Text))
                    {
                        ...
                        buttonLoadscene.Text = "加载 " + item.SubItems[6].Text;
        ...
        {
            if (MessageBox.Show("缓存可提升二次分析的速度，通常无需清除，除非您修改了场景文件。此操作仅清除当前场景的缓存，若需清除所有缓存，请手动删除缓存目录。", "清除缓存", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
