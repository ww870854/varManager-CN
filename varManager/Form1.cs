...
namespace varManager
{
    public partial class Form1 : Form
    {
            ...
            foreach (string varfile in vars)
            {
                if (ComplyVarFile(varfile))
                {
                    ...
                    if (File.Exists(destvarfilename))
                    {
                        ...
                        {
                            this.BeginInvoke(addlog, new Object[] { $"移动 {varfile} 失败，{ex.Message}", LogLevel.ERROR });
                        }
                    }
                    else
                    {
                ...
                {
                    string errlog = $"{varfile} 文件名不符合规则，已移至 {notComplyRuleDirName} 目录";
                    this.BeginInvoke(addlog, new Object[] { errlog, LogLevel.ERROR });
            ...
            if (formUninstallVars.ShowDialog() == DialogResult.OK)
            {
                ...
                foreach (string varname in varimplics)
                {
                    ...
                    if (installedvars.TryGetValue(varname, out linkvar))
                        ...
                        {
                            File.Delete(linkvar);
                            this.BeginInvoke(addlog, new Object[] { $"{varname} 已被卸载。", LogLevel.INFO });
        ...
        private void DeleteVars(List<string> varnames)
        {
            ...
            FormUninstallVars formUninstallVars = new FormUninstallVars();
            formUninstallVars.operationType = "删除";
            ...
            if (formUninstallVars.ShowDialog() == DialogResult.OK)
            {
                ...
                foreach (string varname in varimplics)
                {
                    ...
                    {
                        this.BeginInvoke(addlog, new Object[] { $"{operav} 移动失败，{ex.Message}", LogLevel.ERROR });
                    }
                }
            }
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("当前切换中存在未整理的var文件，请先运行数据更新");
                buttonUpdDB.Focus();
            }
        }
        ...
        {
            string message = "将整理变量、提取预览图片并更新数据库。此过程需要一定时间，敬请耐心等待。";
            ...
            try
            {
                ...
                if (varsrow == null)
                {
                    ...
                    catch (Exception)
                    {
                        ...
                        string errlog = $"{basename}，无效的变量包结构，请移至 {notComplyRuleDirName} 目录";
                    ...
                    if (varnamepart.Length == 3)
                    {
                        ...
                        {
                            this.BeginInvoke(addlog, new Object[] { destvarfilename + " 获取依赖项失败 " + ex.Message, LogLevel.ERROR });
        ...
        private bool UpdDB()
        {
            ...
            foreach (var row in varManagerDataSet.vars)
            {
                ...
                {
                    this.BeginInvoke(addlog, new Object[] { $"{row.varName} 未找到目标VAR文件，该记录将被删除", LogLevel.WARNING });
        ...
        private bool CleanVars(List<string> deletevars)
        {
            try
            {
                this.BeginInvoke(addlog, new Object[] { $"清理依赖关系表...", LogLevel.INFO });
                ...
                dependenciesTableAdapter.Update(varManagerDataSet.dependencies);
                varManagerDataSet.dependencies.AcceptChanges();
                this.BeginInvoke(addlog, new Object[] { $"清理依赖关系表已完成.", LogLevel.INFO });

                this.BeginInvoke(addlog, new Object[] { $"清理场景表...", LogLevel.INFO });
                ...
                scenesTableAdapter.Update(varManagerDataSet.scenes);
                varManagerDataSet.scenes.AcceptChanges();
                this.BeginInvoke(addlog, new Object[] { $"清理场景表完成.", LogLevel.INFO });

                this.BeginInvoke(addlog, new Object[] { $"清理vars表...", LogLevel.INFO });
                ...
                varsTableAdapter.Update(varManagerDataSet.vars);
                varManagerDataSet.vars.AcceptChanges();
                this.BeginInvoke(addlog, new Object[] { $"清理Vars已完成.", LogLevel.INFO });

                this.BeginInvoke(addlog, new Object[] { $"清理预览图片...", LogLevel.INFO });
                foreach (string deletevar in deletevars)
                    DelePreviewPics(deletevar);
                FixPreview();
                this.BeginInvoke(addlog, new Object[] { $"清理预览图片已完成.", LogLevel.INFO });

            }
            catch (Exception ex)
            {
                this.BeginInvoke(addlog, new Object[] { "删除记录或预览错误, " + ex.Message, LogLevel.ERROR });
                return false;
            }
            return true;
        }
        private bool CleanVar(string deletevar)
        {
            ...
            {
                this.BeginInvoke(addlog, new Object[] { deletevar + ",删除记录或预览错误, " + ex.Message, LogLevel.ERROR });
            }
            return true;
        }
        ...
        private int VarInstall(string varName, bool bTemp = false, int operate = 1)
        {
            ...
            if (operate >= 1)
            {
                ...
                if (varsrow != null)
                {
                    ...
                    this.BeginInvoke(addlog, new Object[] { $"{varName}  已安装", LogLevel.INFO });
                    success = 1;
                }
            }
            return success;
        ...
        {
            string message = "分析已安装变量的依赖关系，若在仓库中找到则直接安装，否则将打开处理窗口。";

            const string caption = "缺少依赖项";
            var result = MessageBox.Show(message, caption,
                                         MessageBoxButtons.YesNo,
                                         MessageBoxIcon.Question,
                                         MessageBoxDefaultButton.Button1);
        ...
        {
            string message = "分析所有已组织变量的依赖关系后，将打开一个处理窗口。";

            const string caption = "所有包缺失的依赖项";
            var result = MessageBox.Show(message, caption,
                                         MessageBoxButtons.YesNo,
                                         MessageBoxIcon.Question,
                                         MessageBoxDefaultButton.Button1);
        ...
        public void AllMissingDepends()
        {
            ...
            if (missingvars.Count > 0)
            {
                this.BeginInvoke(addlog, new Object[] { $"总计 {missingvars.Count} 个依赖项缺失", LogLevel.INFO });
                InvokeShowformMissingVars showformMissingVars = new InvokeShowformMissingVars(ShowformMissingVars);
                this.BeginInvoke(showformMissingVars, missingvars);
            }
        }
        ...
        {
            this.BeginInvoke(addlog, new Object[] { "检查已安装的符号链接", LogLevel.INFO });
            ...
            foreach (string linkvar in varfiles)
            {
                try
                {
                    ...
                    if (pathInfo.Attributes.HasFlag(FileAttributes.ReparsePoint))
                    {
                        ...
                        this.BeginInvoke(addlog, new Object[] { $"符号链接 {linkvar} 重建 ...", LogLevel.INFO });
                ...
                {
                    this.BeginInvoke(addlog, new Object[] { linkvar + " 重建符号链接失败。 " + ex.Message, LogLevel.ERROR });
                }
            }
        ...
        private void FixPreview()
        {
            foreach (varManagerDataSet.scenesRow scenerow in this.varManagerDataSet.scenes.Where(q => !string.IsNullOrEmpty(q.previewPic)))
            {
                ...
                if (!File.Exists(picpath))
                {
                    ...
                    {
                        this.BeginInvoke(addlog, new Object[] { $"缺少的 {picpath} 已修复。", LogLevel.INFO });
                    }
                    else
                    {
                        this.BeginInvoke(addlog, new Object[] { $"{picpath} 文件缺失，修复失败", LogLevel.WARNING });
                    }
                }
            }
        ...
        {
            List<string> dependencies = new List<string>();
            this.BeginInvoke(addlog, new Object[] { "分析'Save'目录中的*.json文件以及'Custom'目录中的*.vap文件 ", LogLevel.INFO });
            ...
            foreach (string jsonfile in savefiles)
            {
                ...
                else
                {
                    ...
                    {
                        this.BeginInvoke(addlog, new Object[] { jsonfile + " 获取依赖项失败 " + ex.Message, LogLevel.ERROR });
                    }
                }
            }
            ...
            this.BeginInvoke(addlog, new Object[] { $"{dependencies.Count()} var 文件将被安装", LogLevel.INFO });
            List<string> missingvars = new List<string>();
            foreach (string varname in dependencies)
            {
                string varexistname = VarExistName(varname);
                ...
                {
                    VarInstall(varexistname);
                    this.BeginInvoke(addlog, new Object[] { varexistname + " 已安装", LogLevel.INFO });
                }
                else
                {
                    missingvars.Add(varname);
        ...
        {
            string message = "将分析并修复AddonPackages文件夹中的符号链接，若您的var文件存储库位置发生变更。";

            const string caption = "重建链接";
            var result = MessageBox.Show(message, caption,
                                         MessageBoxButtons.YesNo,
                                         MessageBoxIcon.Question,
                                         MessageBoxDefaultButton.Button1);
        ...
        {
            string message = "分析\"Saves\"文件夹中json文件的依赖关系";

            const string caption = "保存的依赖项";
            var result = MessageBox.Show(message, caption,
                                         MessageBoxButtons.YesNo,
                                         MessageBoxIcon.Question,
                                         MessageBoxDefaultButton.Button1);
        ...
        private void listViewPreviewPics_RetrieveVirtualItem(object sender, RetrieveVirtualItemEventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(curpriviewpic.Picpath))
            {
                ...
                {
                    this.BeginInvoke(addlog, new Object[] { $"{picpath} 文件缺失，请运行 '修复预览' 功能。", LogLevel.WARNING });
        ...
        */
        ...
        void StaleVars()
        {
            ...
            foreach (var oldvar in listOldvar)
            {
                if (varManagerDataSet.dependencies.Count(q => q.dependency == oldvar) == 0)
                {
                    ...
                    try
                    {
                        this.BeginInvoke(addlog, new Object[] { $"将 {oldv} 移动到 {stalepath}。", LogLevel.INFO });
                        File.Move(oldv, stalev);
                        CleanVar(oldvar);
                    }
                    catch (Exception ex)
                    {
                        this.BeginInvoke(addlog, new Object[] { $"{oldv} 移动失败，{ex.Message}", LogLevel.ERROR });
                    }
                }
            }
        ...
        void OldVersionVars()
        {
            ...
            foreach (var oldvar in listOldvar)
            {
                ...
                try
                {
                    this.BeginInvoke(addlog, new Object[] { $"将 {oldv} 移动到 {oldversionpath}。", LogLevel.INFO });
        ...
        private void varsViewDataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (varsViewDataGridView.Columns[e.ColumnIndex].Name == "installedDataGridViewCheckBoxColumn" && e.RowIndex >= 0)
            {
                ...
                if (installed)
                {
                    string message = varName + " 将被删除，确定吗？";
                ...
                {
                    string message = varName + "  将被安装，你确定吗？";
        ...
        private void buttonInstall_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show($"请勿一次安装超过{max}个文件");
                return;
            }
            string message = $"将安装 {varNames.Count} 个变量及其依赖项，请确认是否继续？";
            string caption = "Install Var";
            var result = MessageBox.Show(message, caption,
                                  MessageBoxButtons.YesNo,
                                  MessageBoxIcon.Question,
        ...
        private void listViewPreviewPics_Click(object sender, EventArgs e)
        {
            if (listViewPreviewPics.SelectedIndices.Count >= 1)
            {
                ...
                if (item != null)
                {
                    ...
                    {
                        buttonpreviewinstall.Text = "卸载";
                        buttonpreviewinstall.ForeColor = Color.Red;
                    }

                    else
                    {
                        buttonpreviewinstall.Text = "安装";
                    ...
                    if (item.SubItems[4].Text.ToLower() == "scenes" || item.SubItems[6].Text.ToLower() == "true")
                    {
                        ...
                        buttonLoad.Text = "加载 " + item.SubItems[4].Text;
                        string rescan = "false";
        ...
        {
            string message = "分析日志文件中的依赖关系，否则将打开处理窗口。\r\n此功能需要关闭游戏";
        ...
        private void buttonUninstallSels_Click(object sender, EventArgs e)
        {
            ...
            if (varNames.Count > max)
            {
                MessageBox.Show($"请勿一次卸载超过{max}个文件");
                return;
            }
            string message = $"共有 {varNames.Count} 个变量及其依赖项将被卸载，请确认操作？";
        ...
        private void buttonDelete_Click(object sender, EventArgs e)
        {
            ...
            if (varNames.Count > max)
            {
                MessageBox.Show($"请勿一次性删除超过{max}个文件");
                return;
            }
            string message = $"共有 {varNames.Count} 个变量及其依赖项将被删除，请确认是否执行？";
        ...
        private void varpacksSwitch(string sw)
        {
            ...
            this.BeginInvoke(addlog, new Object[] { $"将Addonpackages符号链接指向 '{sw}'", LogLevel.INFO });
            ...
            if (dipack.Exists)
            {
                if (Comm.ReparsePoint(dipack.FullName).ToLower() != diswitch.FullName.ToLower())
                {
                    ...
                    {
                        MessageBox.Show("错误：创建AddonPackages符号链接。 " +
                                "(错误代码: " + Marshal.GetLastWin32Error() + ")");
                    }
                    else
                    {
        ...
        private void buttonPacksDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                if (MessageBox.Show($"是否要删除 {curswitch} AddonPackagesSwitch？", "delete switch", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    string packsSwitchpath = new DirectoryInfo(Path.Combine(Settings.Default.vampath, addonPacksSwitch)).FullName.ToLower();
        ...
        public List<string> InstallTemp(string[] varNames, ref bool rescan)
        {
            ...
            foreach (string varname in varnames)
            {
                ...
                {
                    this.BeginInvoke(addlog, new Object[] { string.Format("缺少变量:{0},安装失败", varname), LogLevel.INFO });
                }
            }
            return varnames;
            //UpdateVarsInstalled();
        ...
        private void buttonpreviewinstall_Click(object sender, EventArgs e)
        {
            ...
            {
                string message = varName + "  是否确定要安装？";
                string caption = "Install Var";
                var result = MessageBox.Show(message, caption,
                                      MessageBoxButtons.YesNo,
                                      MessageBoxIcon.Question,
        ...
        private void SwitchControl_Enter(object sender, EventArgs e)
        {
            if (ExistAddonpackagesVar())
            {
                if (MessageBox.Show("当前切换中存在未整理的var文件，请先运行数据更新", "警告", MessageBoxButtons.OKCancel) == DialogResult.OK)
                    buttonUpdDB.Focus();
            }
        }
        ...
        {
            string message = "缺失的预览图像将被检测并重新提取。";

            const string caption = "重建预览";
            var result = MessageBox.Show(message, caption,
                                         MessageBoxButtons.YesNo,
                                         MessageBoxIcon.Question,
                                         MessageBoxDefaultButton.Button1);
        ...
        {
            if (MessageBox.Show("缓存可提升二次分析的速度，通常无需清除，除非您修改了场景文件。此操作仅清除当前场景的缓存，若需清除所有缓存，请手动删除缓存目录。", "Clear Cache", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
